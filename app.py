...
# 1. as sidebar menu
with st.sidebar:
    sidebar_selection = option_menu(" Menu", ['Home','Layout parser','Extract Table','Question Answering','Chatboot','Upload json file' ],
        icons=['house','bi bi-columns-gap','table','bi bi-patch-question-fill','bi bi-chat-left-dots-fill','bi bi-filetype-json'], menu_icon="cast", default_index=1)
    sidebar_selection


with st.sidebar:
    sidebar_selection2 = option_menu(None,['contact','Settings' ],
        icons= ['gear','bi bi-person-lines-fill'], menu_icon="info", default_index=1)
    sidebar_selection2
...
if sidebar_selection2 == 'contact':


    selected_1 = option_menu(None, ["Infos", "Message"], 
    icons=['house', 'cloud-upload'], 
    menu_icon="cast", default_index=0, orientation="horizontal")

    if selected_1 == "Message":
    # Method 1: Context Manager Approach (with)
            with st.form(key='form1',clear_on_submit=True):
                st.subheader(':green[Contact]')
                firstname = st.text_input(':blue[Firstname"]')
                lastname = st.text_input(':blue[Lastname]')
                email = st.text_input(':blue[Email]', placeholder='Enter Your Email')
                message = st.text_area(':blue[Message]', placeholder='Enter Your message')
        # Important
